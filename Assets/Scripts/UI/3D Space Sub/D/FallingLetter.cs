...
public class FallingLetter : MonoBehaviour
{
    ...
    {
        Vector3 worldPos = transform.position; // Get world position before unparenting
        gameObject.transform.SetParent(null);   // Now unparent
        transform.position = worldPos;          // Set the position back to the captured world pos
    ...
    {
        yield return new WaitForSeconds(this.fallDelay + this.destroyDelay + 5.0f);
        if (isFalling)
        {
            yield return StartCoroutine(FadeOut());
            Destroy(gameObject);
