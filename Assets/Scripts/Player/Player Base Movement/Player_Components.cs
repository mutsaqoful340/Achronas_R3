...
public class Player_Components : MonoBehaviour
{
    ...
    [Header("Stamina")]
    [SerializeField]
    private int inspectorStaminaCurrent;
    [SerializeField]
    private int inspectorStaminaMax;
    ...
    private Player_InputHandle input;
    ...
    private void Update()
    {
        ...
        // Update stamina on the Rona data object (drain while running)
        if (_rona != null)
        {
            _rona.UpdateStamina(Time.deltaTime);
            // mirror stamina values into serializable inspector fields
            inspectorStaminaCurrent = _rona.Stamina.Current;
            inspectorStaminaMax = _rona.Stamina.Maximum;
        }
    }
    private void Move()
    {
        // state is determined below based on input and running ability
        // Use stamina from the Rona data object
        // (placeholder check — adjust consumption logic as needed)
        if (_rona != null && _rona.Stamina.Current < 0)
        {
            // if stamina is negative, clamp to zero
            _rona.Stamina.Current = 0;
        }
        // Safe input access
        ...
        bool canActuallyRun = runHeld && _rona != null && _rona.Stamina.Current > 0;
        float targetSpeed = isCrouching ? crouchSpeed : (canActuallyRun ? runSpeed : walkSpeed);
        Vector3 desiredVelocity = moveDir * targetSpeed;

        // Update player state: Idle / Walking / Running
        bool isMoving = desiredVelocity.sqrMagnitude > 0.01f;
        if (isMoving)
        {
            if (canActuallyRun && !isCrouching)
                _rona._playerState = Rona.PlayerState.Running;
            else
                _rona._playerState = Rona.PlayerState.Walking;
        }
        else
        {
            _rona._playerState = Rona.PlayerState.Idle;
        }
